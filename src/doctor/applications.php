<?php
/** Displays all CHO applications with search, barangay filter, and pagination. */
session_start();
...
// Build query for all applications - get barangay from draft data (step 1)
$baseSql = "
    SELECT
        a.application_id,
        a.application_date,
...
$countSql = "SELECT COUNT(*) as total FROM application a JOIN applicant ap ON ap.applicant_id = a.applicant_id LEFT JOIN application_draft d ON d.application_id = a.application_id AND d.step = 1 WHERE 1=1";
if (!empty($conds)) {
...
// Status badge color mapping based on workflow_status
function getStatusBadge($status, $workflow_status) {
    // Check final status first
    if ($status === 'Approved') {
        return ['bg' => '#22c55e', 'text' => 'ACCEPTED'];
    }
    if ($status === 'Denied' || $workflow_status === 'denied') {
        return ['bg' => '#f87171', 'text' => 'FEEDBACK'];
    }

    // Check workflow status for pending applications
    switch ($workflow_status) {
        case 'draft':
            return ['bg' => '#9ca3af', 'text' => 'DRAFT'];
        case 'submitted':
            return ['bg' => '#60a5fa', 'text' => 'SUBMITTED'];
        case 'pdao_review':
            return ['bg' => '#a78bfa', 'text' => 'PDAO REVIEW'];
        case 'cho_review':
            return ['bg' => '#f0ad4e', 'text' => 'CHO REVIEW'];
        default:
            return ['bg' => '#f0ad4e', 'text' => 'PENDING'];
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="main">
        ...
        <?php else: ?>
            <?php foreach ($rows as $r):
                ...
                $badge = getStatusBadge($r['status'], $r['workflow_status'] ?? '');
            ?>
            <div class="member-list">
                ...
                <div class="d-flex align-items-center">
                    <span class="badge text-white text-center" style="font-size: 0.85rem; min-width: 110px; padding: 0.5rem 0.75rem; background-color: <?= $badge['bg'] ?>; white-space: nowrap;"><?= $badge['text'] ?></span>
